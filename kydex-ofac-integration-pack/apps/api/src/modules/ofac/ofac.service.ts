...
export class OfacService {
  ...
  async sync(params?: { files?: string[]; force?: boolean; mode?: 'manual' | 'scheduled' | 'startup' }): Promise<unknown> {
    ...
    try {
      for (const file of files) {
        ...
        const parsedEntities = this.parser.parseEntitiesXml(xml, file.includes('CONS') ? 'Consolidated List' : 'SDN List');
