...
export default function DashboardLebanonDownloadsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const importedLists = await getSourceImportedLists(SOURCE_CODE);

      const statuses = await Promise.all(
        importedLists.map(async (list) => {
          try {
            return await getSourceTranslationStatus(SOURCE_CODE, list.listName);
          } catch {
            return null;
          }
        }),
      ...
      setError(
        loadError instanceof Error ? loadError.message : "Failed to load Lebanon imported lists",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDownload = useCallback(
    async (listName: string, format: "csv" | "json") => {
      ...
      try {
        const data = await getSourceListDownload(SOURCE_CODE, listName);
  ...
  return (
    <DashboardShell
      title="تنزيلات اللائحة الوطنية"
      description="قم بتنزيل النسخة المحلية من اللائحة بصيغة CSV أو JSON مع توضيح مستوى التغطية العربية لكل قائمة."
      actions={
    ...
    >
      ...
      {error ? (
        <StateBox tone="error" title="تعذر تحميل قوائم اللائحة الوطنية" detail={error} />
      ) : null}
              ...
              <DashboardCard key={list.id} className="min-w-0">
                <div className="flex items-start justify-between gap-3">
                  <div className="text-right">
                    <p className="text-sm font-medium text-emerald-800">اللائحة الوطنية</p>
