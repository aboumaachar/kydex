...
async function main() {
  const [slug, displayName, apiKey, label] = process.argv.slice(2);

  if (!slug || !displayName || !apiKey) {
    throw new Error(
      'Usage: tsx src/scripts/create-notary-api-key.ts <slug> <displayName> <apiKey> [label]',
    );
  }
  ...
  const record = await prisma.notaryApiKey.create({
    data: {
      ...
      label: label ?? 'Initial webpage key',
      isActive: true,
    },
  });
